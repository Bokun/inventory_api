package io.bokun.inventory.util;
...
/**
 * Converts between various combinations of GRPC (both server and client) and REST representations of the model classes. One of the most boring
 * classes you will ever see in your entire life.
 *
...
public final class GrpcRestMapper {
    ...
    public static io.bokun.inventory.common.api.grpc.BinaryTicket restToGrpc(@Nonnull io.bokun.inventory.plugin.api.rest.BinaryTicket in) {
        return io.bokun.inventory.common.api.grpc.BinaryTicket.newBuilder()
                .setTicketContent(ByteString.copyFrom(BASE64_DECODER.decode(in.getTicketContent())))        // the convention here is to use Base64 encoding for RESTful API as it doesn't support byte arrays
                .build();
    }
